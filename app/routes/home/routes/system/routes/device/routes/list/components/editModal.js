...
export default class EditModal extends BaseComponent{
    ...
    inputChange(e){
        let {selectStates,isFirstModify} = this.state;
        let option  = selectStates.getSelect('devicesType').options
        ...
        if(isFirstModify){
            option.unshift({name:e,value:e})
            this.setState({isFirstModify:false})
        }else {
            option[0] = {name: e, value: e}
        }
        this.handleSelectState('devicesType',{
            options: option,
            isLoading: false,
            value:e
        });
                            ...
                            <tr>
                                ...
                                <td>
                                    <MySelect  onChange={(obj)=>this.handleSelectState('devicesType',obj)}
                                              {...selectStates.getSelect('devicesType')} style={{minWidth:'148px',height:'30px'}}
                                               editable={true} inputChange={(e)=>{this.inputChange(e);
                                               this.handleEditableDataChange('UseType',e)}}
                                    >
